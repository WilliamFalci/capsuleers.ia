...
| **capsuleers.app** (killboard + intel API) | pilot intel (90 days, own archive), batch Local scan, kills/losses, entity battles; lifetime totals (eve-kill behind the site's cache) | live | ✅ on-demand, primary (`capsuleers-api.mjs`, `intel.mjs`) |
| **eve-kill** (eve-kill.com/api) | killboard REST | live | fallback only when capsuleers.app does not answer, plus ticker/partial-name search |
