...
export async function ask(question, onToken = () => {}, uiLang = null) {
  ...
  const directives = [
    ...
    { on: !!mcp.theory,
      it: `\nIl CONTESTO contiene le statistiche calcolate di un fit (eve-fit-engine, parità pyfa). Le STATISTICHE (DPS, gittata, EHP/tank, cap, velocità, targeting) e il FIT in EFT sono già mostrati come SCHEDA GRAFICA e blocco sotto la tua risposta. Scrivi quindi SOLO il **theorycrafting**, in 4-6 frasi: a cosa serve questo fit (ruolo PvP/PvE), il range e la velocità d'ingaggio ideali, i punti di forza e di debolezza, e come si vola/combatte. Puoi citare 1-2 numeri chiave dal contesto per motivare, ma NON elencare tutte le statistiche, NON elencare i moduli e NON produrre un blocco EFT (sono già mostrati). Per le dottrine, ricorda che la rilevazione si basa sulle PERDITE degli ultimi 30 giorni.`,
      en: `\nThe CONTEXT has the computed stats of a fit (eve-fit-engine, pyfa parity). The STATS (DPS, range, EHP/tank, cap, speed, targeting) and the EFT fit are ALREADY shown as a GRAPHIC CARD + block below your answer. So write ONLY the **theorycrafting**, in 4-6 sentences: what this fit is for (PvP/PvE role), the ideal engagement range and speed, its strengths and weaknesses, and how to fly/fight it. You may cite 1-2 key numbers from the context to justify, but do NOT list all the stats, do NOT list the modules and do NOT output an EFT block (they are already shown). For doctrines, note the detection is based on the last 30 days of LOSSES.` },
  ];
